...
import { claimNextGeneration } from "@/lib/generation-queue";
...
async function getStationContext(req: Request, bodyText: string) {
  ...
  if (!auth.ok) return { ok: false as const, response: auth.response };
  if (!auth.context.profile.school_id) {
    return {
      ok: false as const,
      response: NextResponse.json({ error: "School context is required." }, { status: 400 }),
    };
  }
  ...
  return {
    ...
    schoolId: auth.context.profile.school_id,
    classStationId: null,
  };
}
...
export async function POST(req: Request) {
  try {
    ...
    const generation = await claimNextGeneration(context.supabase, {
      schoolId: context.schoolId,
      classStationId: context.classStationId,
    });

    if (!generation) {
      return NextResponse.json({ generation: null, message: "No pending generation jobs." });
    }
