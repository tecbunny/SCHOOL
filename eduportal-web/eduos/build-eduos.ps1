...
if ($compiler) {
    ...
    if (!(Test-Path "build/dtb")) { 
        New-Item -ItemType Directory -Path "build/dtb" -Force 
    }
    & $compiler.Source -I dts -O dtb -o "build/dtb/eduportal.dtbo" "../eduportal.dts"
...
New-Item -ItemType File -Path "eduos-luckfox-pico-ultra.img" -Force | Out-Null
